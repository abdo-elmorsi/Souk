...
import { updateProfileEmail } from '../../../redux/slices/profileSlice'
...
const ProfileVerifyEmail = () => {
    ...
    const onSubmit = (data) => {
        ...
        if (data.code === code) {
            const email = JSON.parse(localStorage.getItem('newEmail'))
            dispatch(updateProfileEmail({ email, token: auth.token }))
        } else {
            setVerifyErr(true)
        }
    }
